...
type Shared = {
   ...
   * @description 服务推送
   */
  pushModalTips(options: ModalOptions): Promise<void>;
  /**
   * @description 设置参数
   * @param schedule
   */
   ...
   * @param schedule
   */
  setPushOptions(options: { nick: string; id: string }): void;
  /**
   * @description 获取用户信息
   */
...
const shared: Shared = {
  ...
  async pushModal(options) {
    ...
    // 推送配置
    const { from, enabled } = PUSH_CONFIG;
    // 启用推送
    if (enabled) {
      // 推送
      await pushModal(
  ...
  async pushModalTips(options) {
    ...
    // 服务推送配置
    const { nick, from, enabled } = PUSH_CONFIG;
    // 启用推送
    if (enabled) {
      // 推送
      await pushModal({ title, subTitle, to: nick, content, type, from });
    }
  },
  setParams(params) {
    if (params) {
